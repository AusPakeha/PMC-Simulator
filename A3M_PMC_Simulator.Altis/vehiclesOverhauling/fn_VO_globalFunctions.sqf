...
THY_fnc_VO_serviceExecution = {
	...
	{
		// Budget check
		if (B_DefenseBudget < 5000) exitWith { hint "OPSG does not have the available company funds to service this vehicle."; };
		...
		if ( (alive _player) && (alive _stat) && (alive _veh) && (_isDroneConnected || ((_player distance _stat) <= _currentRng)) ) then
		{
			switch  _service do {
				case "rep":  // REPAIRING
				{
					...
					private _CurDmg = getDammage _veh;
					...
					B_TotalCost = B_TotalCost + _DmgPrice;
					publicVariable "B_TotalCost";
					publicVariable "B_DefenseBudget";

					// Hint cost
					hint format ["SERVICE DEPARTMENT: \n \n Repairs: $%1 \n \n Billed to OPSG PMC GROUP.", _DmgPrice];
				...
				case "ref":  // REFUELING
				{
					...
					private _LastFuelState = fuel _veh;
					private _ReFuelCost = (1 - _LastFuelState) * 10 * _GasPrice;

					// Update budget
					B_DefenseBudget = B_DefenseBudget - _ReFuelCost;
					B_TotalCost = B_TotalCost + _ReFuelCost;
					publicVariable "B_TotalCost";
					publicVariable "B_DefenseBudget";

					// Hint cost
					hint format ["SERVICE DEPARTMENT: \n \n Refuelling: $%1 \n \n Billed to OPSG PMC GROUP.", _ReFuelCost];
				...
				case "rea":  // REARMING
				{
					// Calculate rearm cost (simplified, based on magazine count)
					private _ReArmCost = 0;
					private _magazines = getArray(configFile >> "CfgVehicles" >> _vehType >> "magazines");
					if (count _magazines > 0) then {
						_ReArmCost = count _magazines * 100;
					};
					...
					if (_count > 0) then {
						for "_i" from 0 to (_count - 1) do {
							...
							private _magazinesTurret = getArray(_config >> "magazines");
							_ReArmCost = _ReArmCost + (count _magazinesTurret * 100);
							private _count_other = count (_config >> "Turrets");
							if (_count_other > 0) then {
								for "_i" from 0 to (_count_other - 1) do {
									private _config2 = (_config >> "Turrets") select _i;
									private _magazinesTurret2 = getArray(_config2 >> "magazines");
									_ReArmCost = _ReArmCost + (count _magazinesTurret2 * 100);
								};
							};
						};
					};
					...
					B_TotalCost = B_TotalCost + _ReArmCost;
					publicVariable "B_TotalCost";
					publicVariable "B_DefenseBudget";

					// Hint cost
					hint format ["SERVICE DEPARTMENT: \n \n Rearmament: $%1 \n \n Billed to OPSG PMC GROUP.", _ReArmCost];
