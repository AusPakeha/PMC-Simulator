...
isNil{
	...
	{
		_f pushBack _x
	}forEach[_bed, _blanket, _chair, _desk, _mw, _fridge, _rack, _shelf, _sofa, _stool, _junk, _tv];
	_b setVariable["PF", _f];

	_bed setPos(_b modelToWorld[-3.6, 1.05, -.6]);
	_blanket setPos(_b modelToWorld[-3.6, 1.1, -.565]);
	_blanket setDir(_dir+270);
	_chair setPos(_b modelToWorld[-1.2, 2.08, -1.03]);
	_chair setDir(_dir+250);
	_desk setPos(_b modelToWorld[.025, 2.16, -.36]);
	_fridge setPos(_b modelToWorld[1.22, 2.14, -.04]);
	_mw setPos(_b modelToWorld[0.5, 2, -.177]);
	_mw setDir(_dir+150);
