...
// ________________set true or false for Debug, you can add this also to an initServer.sqf_____________
...
if (_Debug_info) then {
	[] spawn {
		while { true } do {
			...
			diag_log format ["FPS : %1 || Objects : %2 || allUnits : %3", round diag_fps, count allMissionObjects "All", count allUnits, {
				count _x;
			}];
			systemchat format ["FPS : %1 || Objects : %2 || allUnits : %3", round diag_fps, count allMissionObjects "All", count allUnits, {
				count _x;
			}];
		};
	};
...
// ________________Add here your Exclude Zone (for more)________________
...
// ________________Add here the distance for the Exclude Zone or add your desired________________
// ex:((_x distance GF_Exclude_Zone_1) > 50) // meters

GF_Exclude_Zone_distance= 30;// meters for the test mission

// ________________for continuous run (loop the script) use :________________
while { true } do {
	// ________________for one run only, with a radio trigger or addAction use :________________
	// []spawn{
		...
		if (_Show_info) then {
			onEachFrame {
				hintsilent format ["FPS : %1 || Objects : %2 || allUnits : %3", round diag_fps, count allMissionObjects "All", count allUnits, {
					count _x;
				}]
			}
			// systemchat format ["FPS : %1    ||    Objects : %2    ||    allUnits : %3", round diag_fps, count allMissionObjects "All", count allUnits, {
		...
		if (_1_Blood_Stains) then {
...

			{
				...
				{
					(({
						((_x distance _Delete_this) <= _Delete_distance)
				...
				&& ((_x distance GF_Exclude_Zone_5) > GF_Exclude_Zone_distance)) then {
		...
		if (_10_Empty_Vehicles) then {
...

			{
				...
				(!(_x getVariable ["GF_Excude_from_cleanup", false])) && {
					(({
						((_x distance _Delete_this) <= _Delete_distance)
					} count _allPlayers) isEqualTo 0)
				}
		...
		// ________________ _14_Destruction_Crater _____________it's not working!
