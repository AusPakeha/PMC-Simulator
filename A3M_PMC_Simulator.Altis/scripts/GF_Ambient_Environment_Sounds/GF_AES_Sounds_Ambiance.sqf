...
GF_AES_Sounds_Ambiance_Spawn = {
	...
	[] spawn {
		while { true } do {
			// ________________dayTime and nighttime________________
			...
			if ((date select 3) < 6 or (date select 3) > 19) then {
				...
				// ________________nighttime________________
				...
				if ((!(surfaceIsWater getPos player)) && (!(vehicle player != player))) then {
					...
					// ________________surfaceIsWater________________
				...
				// ________________dayTime________________
