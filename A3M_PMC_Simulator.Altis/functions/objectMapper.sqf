...
if (typeName _pos != "ARRAY" || count _pos < 2) exitWith {diag_log format ["objectMapper: invalid position %1", _pos];};

_objs = [];
_objs = call (compile (preprocessFileLineNumbers format ["%1",_script]));
private ["_posX", "_posY"];
_posX = _pos select 0;
_posY = _pos select 1;
_newObjs = [];
...
for "_i" from 0 to ((count _objs) - 1) do
{
	...
	if (!isNil "_vehicleinit" && {typeName _vehicleinit == "STRING"}) then { _newObj call compile _vehicleinit; };
